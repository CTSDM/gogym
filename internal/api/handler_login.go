...
func (s *State) HandlerLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err := decoder.Decode(&params); err != nil {
		respondWithError(w, http.StatusBadRequest, "Could not decode the request body into a JSON.", err)
		return
	}
	...
	if err := validateLogin(params); err != nil {
		...
		respondWithError(w, http.StatusBadRequest, message, nil)
		return
	}
	...
	if err == pgx.ErrNoRows {
		...
		respondWithError(w, http.StatusInternalServerError, "Something went wrong while processing the login", err)
		return
	}
	...
	if _, err := s.db.CreateRefreshToken(r.Context(), database.CreateRefreshTokenParams{
		Token:     refreshToken,
		ExpiresAt: pgtype.Timestamp{Time: time.Now().Add(time.Hour), Valid: true},
		UserID:    pgtype.UUID{Bytes: user.ID.Bytes, Valid: true},
	}); err != nil {
		respondWithError(w, http.StatusInternalServerError, "Something went wrong while saving the refresh token", err)
		return
	}
